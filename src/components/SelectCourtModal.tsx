...
const getWidthPercentage = (pixelWidth: number): number => (pixelWidth / 329) * (screenWidth * 0.9);
const getHeightPercentage = (pixelHeight: number): number => (pixelHeight / 852) * screenHeight;
...
const SelectCourtModal = ({ visible, onClose, onSave }: SelectCourtModalProps) => {
  const [selectedCourtType, setSelectedCourtType] = useState<CourtType>('highCourt');
  ...
  const handleSave = () => {
    ...
    if (selectedCourtType === 'highCourt') {
        if (!selectedHighCourt || !selectedBench) {
            Toast.show({type: 'error', text1: 'Input Required', text2: 'Please select a High Court and a Bench.'});
            return;
        }
        dataToSave = {...dataToSave, highCourt: selectedHighCourt, bench: selectedBench};
    } else if (selectedCourtType === 'districtCourt') {
        if (!selectedState || !selectedDistrict) {
            Toast.show({type: 'error', text1: 'Input Required', text2: 'Please select a State and a District Court.'});
            return;
        }
        dataToSave = {...dataToSave, state: selectedState, district: selectedDistrict};
    }
    onSave(dataToSave);
    onClose();
  };

  const renderDropdown = (placeholder: string, value: string | null, onSelect: (val: string) => void) => (
     <LinearGradient
     ...
     >
        <TouchableOpacity style={styles.dropdownInner} onPress={() => onSelect('Mock Selection')}>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <View style={styles.supremeCourtContainer}>
            ...
            <LinearGradient colors={['#01B779', '#008C68']} style={styles.comingSoonBanner}>
          ...
          <View style={styles.toggleContainer}>
            {['highCourt', 'districtCourt'].map((type) => {
                ...
                const label = type === 'highCourt' ? 'High Court' : 'District Court';
                ...
                return (
                    <TouchableOpacity key={type} onPress={() => setSelectedCourtType(type as CourtType)}>
                        <LinearGradient colors={['#016361', '#01B779']} style={styles.toggleButtonBorder}>
          ...
          {selectedCourtType === 'highCourt' && (
            <View style={styles.dropdownsContainer}>
                {renderDropdown('Select High Court', selectedHighCourt, setSelectedHighCourt)}
          ...
          {selectedCourtType === 'districtCourt' && (
            <View style={styles.dropdownsContainer}>
                ...
                {renderDropdown('Select District Court', selectedDistrict, setSelectedDistrict)}
            </View>
          )}

          <View style={{flex: 1}} />
          <TouchableOpacity onPress={handleSave}>
            <LinearGradient colors={['#01B779', '#008C68']} start={{x: 0, y: 0.5}} end={{x: 1, y: 0.5}} style={styles.saveButton}>
