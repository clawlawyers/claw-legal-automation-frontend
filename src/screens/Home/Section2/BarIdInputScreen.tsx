// src/screens/Home/Section2/BarIdInputScreen.tsx
...
const BarIdInputScreen = (props : BarIdInputScreenNavigationProp) => {
  ...
  const handleFetchCase = () => {
    ...
    if (!barId.trim()) {
        Toast.show({type: 'error', text1: 'Input Required', text2: 'Please enter an Advocate Bar ID.'});
        return;
    }
    
    console.log(`Fetching case for Bar ID: ${barId}`);
    console.log('With saved court details:', selectedCourt);
       navigation.navigate('CaseLoadingScreen', { fromScreen: 'BarIdInputScreen' });
        ...
        <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <View style={styles.innerContainer}>
            <ScrollView contentContainerStyle={styles.scrollContentContainer} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              <Text style={styles.titleText}>Search Using BAR ID</Text>
              <Text style={styles.instructionText}>
                Enter Valid Advocate Bar ID To Fetch Your Case And Add To Your Cases
              </Text>

              {/* Advocate Bar ID Input */}
              <LinearGradient colors={['#016361', '#01B779']} style={styles.inputGradientBorder}>
                <TextInput
                  style={styles.textInput}
                  placeholder="Enter Advocate Bar ID"
...
export default BarIdInputScreen;