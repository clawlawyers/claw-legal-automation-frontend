// src/screens/Home/Section2/FilingNumberInputScreen.tsx
...
const FilingNumberInputScreen = () => {
  ...
  // State for modal and court data
  ...
  const handleFetchCase = () => {
    ...
    if (!filingNumber.trim()) {
        Toast.show({type: 'error', text1: 'Input Required', text2: 'Please enter the Filing Number.'});
        return;
    }
    if (!filingYear.trim() || !/^\d{4}$/.test(filingYear.trim())) {
        Toast.show({type: 'error', text1: 'Input Required', text2: 'Please enter a valid 4-digit Filing Year.'});
        return;
    }
    
    console.log(`Fetching case: Filing No - ${filingNumber}, Year - ${filingYear}`);
    console.log('With saved court details:', selectedCourt);
       navigation.navigate('CaseLoadingScreen', { fromScreen: 'FilingNumberInputScreen' });
        ...
        <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <View style={styles.innerContainer}>
            <ScrollView contentContainerStyle={styles.scrollContentContainer} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              <Text style={styles.titleText}>Search Using Filing Number</Text>
              <Text style={styles.instructionText}>
                Enter Valid Filing Details To Fetch Your Case And Add To Your Cases
              </Text>
...
const styles = StyleSheet.create({
  ...
  inputGradientBorder: {
    ...
    borderRadius: 10,
    marginTop: getHeightPercentage(22), 
    padding: 1.5,
  },
  textInput: {
    width: '100%',
...
export default FilingNumberInputScreen;